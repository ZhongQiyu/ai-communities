...
# from agentscope.agents.audio_user_agent import audio2text
...
def run_app():
    ...
    old_cwd = os.getcwd()
    # Change the current working directory to the directory where script_path is located.
    os.chdir(script_dir)


    def import_function_from_path(module_path, function_name,
                                  module_name=None):
    ...
    with gr.Blocks(css="assets/app.css") as demo:
        ...
        def send_player_reset_message(uid):
            uid = check_uuid(uid)
        ...
        reset_button.click(send_reset_message, inputs=[uuid]).then(send_player_reset_message, inputs=[uuid], outputs=user_chat_input)
